...
__global__ void kernel_SeparableConvolution_updateOutput(const int n,const float* input, const long4 input_size, const long4 input_stride,
	const float* vertical, const long4 vertical_size, const long4 vertical_stride,const float* horizontal,const long4 horizontal_size,
...
void SeparableConvolution_kernel_forward(THCState* state,THCudaTensor* input,THCudaTensor* vertical,THCudaTensor* horizontal,THCudaTensor* output) {
	int n = 0;

	n = THCudaTensor_nElement(state, output);
	kernel_SeparableConvolution_updateOutput<<< (n + 512 - 1) / 512, 512, 0, THCState_getCurrentStream(state) >>>(n,
...
__global__ void kernel_SeparableConvolution_updateGradVertical(const int n,const float* gradLoss, const long4 gradLoss_stride,
    const float* input, const long4 input_stride,const float* horizontal, const long4 horizontal_stride,float* gradVertical,
	const long4 gradVertical_size,const long4 gradVertical_stride) {
	int intIndex = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void kernel_SeparableConvolution_updateGradHorizontal(const int n,const float* gradLoss, const long4 gradLoss_stride,
const float* input, const long4 input_stride,const float* vertical, const long4 vertical_stride,float* gradHorizontal,
 const long4 gradHorizontal_size, const long4 gradHorizontal_stride) {
	int intIndex = blockIdx.x * blockDim.x + threadIdx.x;
	if (intIndex >= n) {
		return;
	}
...
void SeparableConvolution_kernel_backward(THCState* state,THCudaTensor* gradLoss,THCudaTensor* input,THCudaTensor* vertical,
    THCudaTensor* horizontal,THCudaTensor* gradInput,THCudaTensor* gradVertical,THCudaTensor* gradHorizontal) {
    int n = 0;

    cudaStream_t stream = THCState_getCurrentStream(state);
    n = THCudaTensor_nElement(state, gradVertical);
    kernel_SeparableConvolution_updateGradVertical<<< (n + 512 - 1) / 512, 512, 0, THCState_getCurrentStream(state) >>>(n,THCudaTensor_data(state, gradLoss),
	make_long4(gradLoss->stride[0], gradLoss->stride[1], gradLoss->stride[2], gradLoss->stride[3]),THCudaTensor_data(state, input),
	make_long4(input->stride[0], input->stride[1],input->stride[2], input->stride[3]),
	THCudaTensor_data(state, horizontal),
	make_long4(horizontal->stride[0], horizontal->stride[1],horizontal->stride[2], horizontal->stride[3]),
	THCudaTensor_data(state, gradVertical),
	make_long4(gradVertical->size[0], gradVertical->size[1],gradVertical->size[2], gradVertical->size[3]),
	make_long4(gradVertical->stride[0], gradVertical->stride[1],gradVertical->stride[2], gradVertical->stride[3]));

    n = THCudaTensor_nElement(state, gradHorizontal);
    kernel_SeparableConvolution_updateGradHorizontal<<< (n + 512 - 1) / 512, 512, 0, THCState_getCurrentStream(state) >>>(n,
		THCudaTensor_data(state, gradLoss),
    	make_long4(gradLoss->stride[0], gradLoss->stride[1],gradLoss->stride[2], gradLoss->stride[3]),
    	THCudaTensor_data(state, input),
    	make_long4(input->stride[0], input->stride[1],input->stride[2], input->stride[3]),
    	THCudaTensor_data(state, vertical),
    	make_long4(vertical->stride[0], vertical->stride[1],vertical->stride[2], vertical->stride[3]),
    	THCudaTensor_data(state, gradHorizontal),
    	make_long4(gradHorizontal->size[0], gradHorizontal->size[1],gradHorizontal->size[2], gradHorizontal->size[3]),
    	make_long4(gradHorizontal->stride[0], gradHorizontal->stride[1],gradHorizontal->stride[2], gradHorizontal->stride[3]));
